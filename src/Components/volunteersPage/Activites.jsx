...
function Volunteers(props) {
  ...
  const applyHandler = async (id) => {
    try {
      const response = await axios.patch(
        `https://volunteering-b71e0-default-rtdb.europe-west1.firebasedatabase.app/volunteers/${id}.json`,
        { applied: true }
      );
  ...
  const updateHandler = async (activity) => {
    try {
      const response = await axios.patch(
        `https://volunteering-b71e0-default-rtdb.europe-west1.firebasedatabase.app/volunteers/${activity.id}.json`,
        activity
      );
      setActivities((currentState) => {
